...
let authorizeInput (m:LoginModel):HttpHandler=
    printfn "Authorizing some input"
    (fun next ctx ->
        printfn "We are running signin!"
        match m.username with
        | null | "" -> RequestErrors.BAD_REQUEST "Username was not included in post" next ctx
        | un ->
                ...
                match pretendAuthentication un m.password with
                // not security minded, but may be helpful for diagnostics while learning
                | false -> RequestErrors.UNAUTHORIZED authScheme realm (sprintf "User %s is not authorized" un) next ctx
                | true ->
...
// actual handler
let signin :HttpHandler = Giraffe.ModelBinding.bindModel<LoginModel> None authorizeInput

module _LowLevelExample =
    ...
    let (|NoValue|ASingleValue|MultipleValues|) (svOpt:Microsoft.Extensions.Primitives.StringValues option)=
        ...
        |> function
            |Some x -> x
            |None -> NoValue
    // example low-level, not used
    let _manualFormSigninModel:HttpHandler =
        (fun next ctx ->
            printfn "We are running signin!"
            task{
